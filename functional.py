...
class Solver:
    ...
    def competitive_learning_algorithm3(self, S, m1, m2, m3, eta, epoch, x, test_x=None):
    ...
    def competitive_learning_algorithm2(self, S, m1, m2, eta, epoch, x, test_x=None):
        table = PrettyTable(('Iteration', 'x', '||x-m1||', '||x-m2||', 'j', 'mj'))
        table.title = "--- competitive learning algorithm (without normalisation) ---"
        result = []
        for epochi in range(epoch):
            for i in range(np.size(x, 1)):
                x_m1 = np.linalg.norm(x[:, i] - m1)
                x_m2 = np.linalg.norm(x[:, i] - m2)
                if x_m1 == min(x_m1, x_m2):
                    j = 1
                    m1 = m1 + eta * (x[:, i] - m1)
                    result.append((i + 1, x[:, i], x_m1, x_m2, j, 'm' + str(j) + str(m1)))
                elif x_m2 == min(x_m1, x_m2):
                    j = 2
                    m2 = m2 + eta * (x[:, i] - m2)
                    result.append((i + 1, x[:, i], x_m1, x_m2, j, 'm' + str(j) + str(m2)))
        for row in result:
            table.add_row(row)
        table.add_row(['m1:', m1, 'm2:', m2, '',''])
        print(table)
        clazz = []
        for i in range(np.size(S, 1)):
            x_m1 = np.linalg.norm(S[:, i] - m1)
        ...
        if test_x is not None:
            ...
            test_x_m2 = np.linalg.norm(test_x - m2)
            clazz = 0
            if test_x_m1 == min(test_x_m1, test_x_m2):
                clazz = 1
            elif test_x_m2 == min(test_x_m1, test_x_m2):
                clazz = 2
            table = PrettyTable(('test_x', '||test_x-m1||', '||test_x-m2||', 'class'))
            table.add_row([test_x, test_x_m1, test_x_m2, clazz])
            print(table)
